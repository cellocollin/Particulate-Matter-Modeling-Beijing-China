...
Pollution_Daily <- read.csv("pollutiondaily.csv",header=TRUE)
Pollution_Daily
Pollution_Daily$Year=year(Pollution_Daily$Date_D)
Pollution_Daily$Month=month(Pollution_Daily$Date_D)
Pollution_Daily$Day=day(Pollution_Daily$Date_D)
...
plot(1:1789,Pollution_Dailynona$Avg_Day_PM2.5)
Pollution_Daily
pm=ggplot(Pollution_Daily, aes(Date_D, Avg_Day_PM2.5)) +geom_line()
...
```
As we see the variation between the month year and day is important and vary so therefore for the sake of fitting a model between these years every variable will be important
```{r , echo = FALSE }
##Fitting Full Model
