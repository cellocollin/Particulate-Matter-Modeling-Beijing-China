...
Now that we know the distributions we can take a look at some pm2.5 vs predictor and also look at how correlated the three dew precipitation and temp are related
...
plot(Pollution_Daily$Avg_Day_TEMP,Pollution_Daily$Avg_Day_DEWP,xlab='day')
abline(tempdew)
plot(Pollution_Daily$Avg_Day_TEMP,Pollution_Daily$Avg_Day_PRES,xlab='day')
abline(temppres)

plot(1:1826,Pollution_Daily$Avg_Day_TEMP,type="l",xlab="day")
plot(1:1826,Pollution_Daily$Avg_Day_DEWP,type='l',xlab='day')
plot(1:1826,Pollution_Daily$Avg_Day_PRES,type='l',xlab='day')
```
...
One thing is that since there are random fluctations in our final model we will not use date since in a regression model the advance of one day would theoretically advance pm2.5 which is not the relationship we see
...
lm_all=lm(Avg_Day_PM2.5~.-Date_D,data=Pollution_Daily.train)
lm_all
...
anova(lm_all)$Pr
```
Through sequential anova we can determine that our predictor variable Date_D is not significant, therefore we can remove it.  
...
bptest(lm_all_2)
##Testing Normality
shapiro.test(residuals(lm_all_2))
...
```{r echo=FALSE}
test_outlier(lm_all_2)
# rstudent(lm_all_2)
```
As we can see above, we also have four outliers in the dataset. We can remove these outliers from our dataset and refit the model, and see how this changes our diagnostics.
...
Pollution_Daily.train.new<-Pollution_Daily.train[-c(333,886,1181,1213),]
lm_all_3=lm(Avg_Day_PM2.5~.-Date_D,data=Pollution_Daily.train.new)
```

```{r echo=FALSE}
test_outlier(lm_all_3)
```

```{r echo=FALSE}
boxcox(lm_all_3,plotit=T)
par(mfrow=c(2,2))
bc=boxcox(lm_all_3);
