...
```{r echo = FALSE }
box=boxcox(lm_all);box$x[which.max(box$y)]
#summary(lm_all)
lm33=lm(Avg_Day_PM2.5~.-(Year+Month+Day+Max_Day_CBWD) , data = trainingdate)
bc=boxcox(lm33)
lambda=bc$x[which.max(bc$y)]
lmboxfinalmodel <- lm((Avg_Day_PM2.5^lambda-1)/lambda ~Avg_Day_DEWP+Avg_Day_TEMP+Avg_Day_PRES+Max_Day_LWS+Max_Day_LS+Max_Day_LR+factor(Month) , data = trainingdate)
bptest(lmboxfinalmodel)
shapiro.test(residuals(lmboxfinalmodel))
...
rmse(finalmodelpredict,nonatest [ , 2 ] )
```
