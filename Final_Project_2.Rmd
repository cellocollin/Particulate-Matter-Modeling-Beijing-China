...
The pm and time is not a clear relationship and there are many seasonal spikes - One thing is that since there are random fluctations in our final model we will not use date since in a regression model the advance of one day would theoretically advance pm2.5 which is not the relationship we see
...
So in order to remedy non linearity, heteroscedasticity, non normal residual and autocorrelation we can do a variety of transformations

The first we can do is Boxcox for normal residual and linearity
...
This is the boxcox likelihood for lambda and we see that lambda is maximized at 0.14 so that will be the transformation we use
```{r echo=FALSE}
test_outlier(lm_all)
```
...
The shapiro wilk shows that normality has been improved by a lot and data is more homoscedastic.
...
testgen=predict(general,Pollution_Daily.test [ , - c(1,2) ] )
...
plot(c(1:366) ,  testpm , type = "l" , lwd= "2" , col = "red" )
lines( c(1:366),Pollution_Daily.test[,2])
data.frame(rmse(Pollution_Daily.test[,2] , testgen),
rmse(Pollution_Daily.test[,2] , testpm))
rmse(predict(general,Pollution_Daily.test [ , - 1 ]  ) , na.omit( Pollution_Daily.test[ , 2 ] ))
