...
```{r echo = FALSE }
#summary(lm_all)
lmboxfinalmodel <- lm((Avg_Day_PM2.5^lambda-1)/lambda ~Avg_Day_DEWP+Avg_Day_TEMP+Avg_Day_PRES+Max_Day_LWS+Max_Day_LS+Max_Day_LR+factor(Month)+factor(Day)+factor(Month) , data = trainingdate)
finalmodelpredict<-predict(lmboxfinalmodel,nonatest);
diagnostic<-data.frame(rmse(finalmodelpredict,nonatest [,2]),bptest(lmboxfinalmodel)[4],
shapiro.test(residuals(lmboxfinalmodel))[2] ,extractAIC(lmboxfinalmodel)[2]
)
colnames(diagnostic)<-c('rmse', 'bptest p' , 'shapiro p ' , 'aic')
diagnostic
#finalmodelpredict <- predict(lmboxfinalmodel , nonatest)
for (k in 1:355)
{
...
We see that this model has normal residual as well as heteroscedasticity. However there is autocorrelation so a non parametric method such as regression splines or kernel estimation may be a better way to model particulates in Beijing though our model is greatly improved from the previous. 
