...
library(corrplot);corrplot(cor(nump),method='circle')
```
...
```{r echo =FALSE}
library(ggpubr);par(mfrow=c(3,3))
...
tmp=lm(r.ydew ~ r.dew )
partialregression<-data.frame(fullmodel=1:13,partco=1:13)


partialregression[,1][1]=summary(lm_all)$coef[3]
  partialregression[,2][1]=summary(tmp)$coef[2]
partialregression

r.ydate=update(lm_all , ~. -Date_D)$res
r.date=lm(Date_D ~ . , data = nonatrain[ , - 2 ])$res
tmpdate=lm(r.ydate~r.date)
partialregression[,1][2]=summary(lm_all)$coef[2]
  partialregression[,2][2]=summary(tmp1)$coef[2]
  partialregression
...
r.lws=lm(Max_Day_WLS ~.,data=nonatrain[,-2])$res
tmplws=lm(r.ylws~r.wls)
